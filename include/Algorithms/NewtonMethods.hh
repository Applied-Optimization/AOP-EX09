        ...
        * \param _max_iters maximum iteration of the method*/
        ...
        static Vec solve_equality_constrained_with_infeasible_start(FunctionBaseSparse *_problem, const Vec& _initial_x, const SMat &_A, const Vec &_b,
            ...
            while (iter <_max_iters) {
                ...
                // Resolution system checking
                solver.compute(K);
                if(solver.info() != Eigen::Success) {
                    std::cerr << "KKT system solve failed!" << std::endl;
                    break;
                }
                ...
                dxl = solver.solve(rhs);
                if(solver.info() != Eigen::Success){
                    std::cerr <<"Failed to solve KKT system!" << std::endl;
                    break;
                }
                ...
                res = std::sqrt(rdual.squaredNorm() + rpri.squaredNorm());
                ...
                // Line search step
                double t = LineSearch::backtracking_line_search_newton_with_infeasible_start<FunctionBaseSparse>(
                    _problem,
                    _A,
                    _b,
                    x,
        ...
        static Vec solve_equality_constrained_hybrid(FunctionBaseSparse *_problem, const Vec& _initial_x, const SMat &_A, const Vec &_b,
            ...
            while (iter < _max_iters) {   
                // Check feasibility
                rpri = _A * x - _b;
                double constraint_violation = rpri.squaredNorm();
                double t = 1.0;

                if (constraint_violation > _eps_constraints) {
                    // Mode infaisable
                    _problem->eval_gradient(x, g);
                    Anu = _A.transpose() * nu;
                    rdual = g + Anu;
                    ...
                    solver.compute(K);
                    if(solver.info() != Eigen::Success) {
                        std::cerr << "KKT system solve failed in infeasible mode!" << std::endl;
                        break;
                    }
        
                    dxl = solver.solve(rhs);
        
                    // Extraction des variables primales et duales
                    dx = dxl.head(n);
                    dnu = dxl.tail(p);
                    ...
                    // Vérifier la convergence sur le résidu
                    if (res < _eps)
                        break;
            
                    // Line search avec méthode infaisable
                    double t = LineSearch::backtracking_line_search_newton_with_infeasible_start<FunctionBaseSparse>(
                        _problem, _A, _b, x, nu, dx, dnu, res, 1.0);
        
                } else {
                    // Mode faisable standard
                    _problem->eval_gradient(x, g);
                    _problem->eval_hessian(x, H);
                    ...
                    solver.compute(K);
                    if(solver.info() != Eigen::Success) {
                        std::cerr << "KKT system solve failed in feasible mode!" << std::endl;
                        break;
                    }
                    ...
                    // Vérifier la convergence sur le décrément de Newton
                    double lambda2 = -g.transpose() * dx;
                    if (lambda2 <= eps2)
                        break;
            
                    // Line search standard
