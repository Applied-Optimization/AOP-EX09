    ...
    public:
        ...
        * \return the final step t computed by the back-tracking line search */
        ...
        static double backtracking_line_search_newton_with_infeasible_start(Problem *_problem,
            ...
            // Verify step size is positive
            if(t <= 0) {
                std::cerr << "Initial step size must be positive" << std::endl;
                return 0;
            }
            ...
            // Use provided initial residual if available
            double res_initial = _initial_res;
            if(res_initial <= 0) {
                Vec Anu = _A.transpose() * _nu;
                Vec rdual = g + Anu;
                Vec rpri = _A * _x - _b;
                res_initial = std::sqrt(rdual.squaredNorm() + rpri.squaredNorm());
            }
            ...
            while (iter < max_iter) {
                ...
                Vec x_new = _x + t * _dx;
                Vec nu_new = _nu + t * _dnu;
        
                _problem->eval_gradient(x_new, g);
                Vec Anu_new = _A.transpose() * nu_new;
                Vec rdual_new = g + Anu_new;
                Vec rpri_new = _A * x_new - _b;
        
                double res_new = std::sqrt(rdual_new.squaredNorm() + rpri_new.squaredNorm());

                // Check if sufficient decrease condition is satisfied
                if (res_new <= (1 - _alpha * t) * res_initial) {
                    break;
                }
